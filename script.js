...
// -------------------------------------------------------------------
// B. JAVASCRIPT LOGIC (REMAINS THE SAME)
// -------------------------------------------------------------------
...
// Function to render the navigation menu in the sidebar
function renderNavigation() {
    // Clear previous content
    sidebar.innerHTML = '<h2>Agile Methodologies</h2>'; 
    
    const navList = document.createElement('ul');
    navList.classList.add('nav-section');

    for (const methodologyName in methodologiesData) {
        if (methodologiesData.hasOwnProperty(methodologyName)) {
            const listItem = document.createElement('li');
            const link = document.createElement('a');

            link.href = '#';
            link.textContent = methodologyName;
            
            // Set a data attribute to easily identify the methodology
            link.dataset.methodology = methodologyName;

            // Add an event listener to handle clicks and load content
            link.addEventListener('click', (event) => {
                event.preventDefault();
                loadContent(methodologyName, link);
            });
    ...
    // Append the created list to the sidebar
    sidebar.appendChild(navList);
}


// Function to load and render the content for the selected methodology
function loadContent(methodologyName, clickedLink) {
    ...
    let contentHTML = `
        <h1>${methodologyName}</h1>
        <p>${data.description}</p>
        <hr>
    `;

    // Iterate through the expertise levels (or general sections for Introduction)
    for (const sectionKey in data.sections) {
        const section = data.sections[sectionKey];
        contentHTML += `
            <div class="level-section">
                <h3>${section.title} (${sectionKey.replace(/([A-Z])/g, ' $1').trim()})</h3>
                ${section.content}
            </div>
        `;
    }
...
// Initialize the application by rendering the navigation and loading the first item (Introduction)
document.addEventListener('DOMContentLoaded', () => {
    renderNavigation();

    // Automatically load the first methodology on startup (which is now 'Introduction')
    const firstMethodology = Object.keys(methodologiesData)[0];
